...
if __name__ == "__main__":
    ...
    for sample in samples:
        # freebayes_job = Job.wrapJobFn(freebayes.freebayes_single, config, sample, sample['bam'],
        #                               cores=1,
        #                               memory="{}G".format(config['freebayes']['max_mem']))
        #
        # mutect_job = Job.wrapJobFn(mutect.mutect_single, config, sample, sample['bam'],
        #                            cores=1,
        #                            memory="{}G".format(config['mutect']['max_mem']))

        vardict_job = Job.wrapJobFn(vardict.vardict_single, config, sample, samples, sample['bam'],
                                    cores=int(config['vardict']['num_cores']),
                                    memory="{}G".format(config['vardict']['max_mem']))

        scalpel_job = Job.wrapJobFn(scalpel.scalpel_single, config, sample, samples, sample['bam'],
                                    cores=int(config['scalpel']['num_cores']),
                                    memory="{}G".format(config['scalpel']['max_mem']))

        # indelminer_job = Job.wrapJobFn(indelminer.indelminer_single, config, sample, sample['bam'],
        #                                cores=1,
        ...
        # platypus_job = Job.wrapJobFn(platypus.platypus_single, config, sample, sample['bam'],
